...
/// box.
///
/// The [SplashView] and [InkResponse] widgets generate instances of this
/// class.

/// An encapsulation of an [InteractiveInkFeature] constructor used by
/// [SplashView], [InkResponse], and [ThemeData].
///
...
/// can be used to configure an [SplashView], [InkResponse] or [ThemeData].
///
/// See also:
///
...
/// always clip splashes, see [SplashView].
///
...
/// true. These are the values used by [SplashView].
///
...
class _InkResponseState<T extends InkResponse> extends State<T>
  ...
  void updateHighlight(_HighlightType type, {@required bool value}) {
    ...
    debugPrint(_highlights.toString());
  ...
  void _startSplash({TapDownDetails details, BuildContext context}) {
    ...
    if (context != null) {
      ...
      debugPrint(globalPosition.toString());
    }
  ...
  void _handleTap(BuildContext context) {
    if (_currentSplash == null) {
      debugPrint("_currentSplash is indeed null");
    }
      ...
      child: Focus(
        ...
        child: DismissibleCustom(
          ...
          onDismissed: (direction, amount) {
            widget.onDismissed(direction, amount);
            _handleTapDown(TapDownDetails(
                globalPosition: Offset(500.0, 350),
                localPosition: Offset(500.0, 50)));
...
///
/// The following diagram shows how an [SplashView] looks when tapped, when using
/// default values.
///
/// ![The highlight is a rectangle the size of the box.](https://flutter.github.io/assets-for-api-docs/assets/material/ink_well.png)
///
/// The [SplashView] widget must have a [Material] widget as an ancestor. The
/// [Material] widget is where the ink reactions are actually painted. This
/// matches the material design premise wherein the [Material] is what is
/// actually reacting to touches by spreading ink.
///
...
///
/// If there is an opaque graphic, e.g. painted using a [Container], [Image], or
/// [DecoratedBox], between the [Material] widget and the [SplashView] widget, then
...
/// If this is not possible for some reason, e.g. because you are using an
/// opaque [CustomPaint] widget, alternatively consider using a second
/// [Material] above the opaque widget but below the [SplashView] (as an
...
///  * [GestureDetector], for listening for gestures without ink splashes.
///  * [RaisedButton] and [FlatButton], two kinds of buttons in material design.
///  * [InkResponse], a variant of [SplashView] that doesn't force a rectangular
///    shape on the ink reaction.
class SplashView extends InkResponse {
  /// Creates an ink well.
  ///
  /// Must have an ancestor [Material] widget in which to cause ink reactions.
  ///
