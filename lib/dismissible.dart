...
/// the given `direction`.
///
/// Used by [DismissibleCustom.onDismissed].
typedef DismissDirectionCallback = void Function(
    DismissDirection direction, double extent);

/// Signature used by [DismissibleCustom] to give the application an opportunity to
/// confirm or veto a dismiss gesture.
///
/// Used by [DismissibleCustom.confirmDismiss].
typedef ConfirmDismissCallback = Future<bool> Function(
    DismissDirection direction);

/// The direction in which a [DismissibleCustom] can be dismissed.
...
///
///
typedef DismissMoveCallback = void Function(double extent);

class DismissibleCustom extends StatefulWidget {
  /// Creates a widget that can be dismissed.
  ///
  /// The [key] argument must not be null because [DismissibleCustom]s are commonly
  /// used in lists and removed from the list when dismissed. Without keys, the
  /// default behavior is to sync widgets based on their index in the list,
  /// which means the item after the dismissed item would be synced with the
  /// state of the dismissed item. Using keys causes the widgets to sync
  /// according to their keys and avoids this pitfall.
  const DismissibleCustom({
    @required Key key,
    @required this.child,
    this.background,
    ...
    this.resizeDuration = const Duration(milliseconds: 600),
    this.dismissThresholds = const <DismissDirection, double>{},
    this.movementDuration = const Duration(milliseconds: 600),
  ...
  final DismissMoveCallback onMove;
  ...
  @override
  _DismissibleCustomState createState() => _DismissibleCustomState();
}
...
class _DismissibleCustomState extends State<DismissibleCustom>
  ...
  Future<void> _handleDragEnd(DragEndDetails details) async {
    ...
    if (_moveController.value >
      ...
      widget.onDismissed(direction, _dragExtent);
    }
    final double flingVelocity = _directionIsXAxis
        ? details.velocity.pixelsPerSecond.dx
        : details.velocity.pixelsPerSecond.dy;
    debugPrint('describing fling gesture');
    switch (_describeFlingGesture(details.velocity)) {
      case _FlingGestureKind.forward:
        debugPrint('describing fling gesture forward');
        assert(_dragExtent != 0.0);
        assert(!_moveController.isDismissed);
        _moveController.reverse();
        // _moveController.duration = Duration(milliseconds: 500);
      ...
      case _FlingGestureKind.reverse:
        debugPrint('describing fling gesture reverse');
        assert(_dragExtent != 0.0);
        assert(!_moveController.isDismissed);
        _dragExtent = flingVelocity.sign;
        _moveController.fling(
